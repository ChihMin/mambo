...
bool inline_uncond_imm(dbm_thread *thread_data, scanner_queue_t *queue, bool insert_branch,
                       uint32_t **write_p, uint32_t **read_addr, uint32_t target, int *inlined_back_count) {
    ...
    switch(inst) {
      ...
      case ARM_BL: {
        ...
        if (condition_code == AL) {
          thread_data->code_cache_meta[basic_block].exit_branch_addr = (uint16_t *)write_p;
          if (!inline_uncond_imm(thread_data, scan_queue, true, &write_p, &read_address, target, &inlined_back_count)) {
